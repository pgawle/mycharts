...
VAP.zones = function (width, height, groups) {
...
VAP.zones.prototype = {
    ...
    add: function (container, groups) {
        ...
        var zones = container.selectAll(".zone")
            ...
            .attr('class', function (d) {
                //console.log(d);
                var className = 'zone zone' + d.key;
                if (d.css_style !== undefined) {
                    className = className + " " + d.css_style;
                }
                return className;
            })
    ...
    tick: function (svgObj, zones, zones_link_chart) {
        ...
        zones.attr("d", function (d) {
            ...
            var label = svgObj.select('#label' + d.key)
            label.attr('x', pos.x);
            ...
            if (zones_link_chart) {
                ...
                zones_link_chart.nodes().forEach(function (node) {
                    //console.log(node);
                    if (node.zone_id === parseInt(d.key, 10)) {
                        node.x = pos.x;
                        node.y = pos.y + 50;
                    }
                });

                //console.log(zone_center);
            }
            //var zone_center = zones_link_chart.force_chart.nodes()[d.key];
            //console.log(zone_center);
            //var path
    ...
    concetrateNodesInZones: function (tick_event, nodes) {
        ...
        nodes.forEach(function (o, i) {
            if (o.zone_id !== undefined) {
                o.y += (that.center_points[o.zone_id].y - o.y ) * k;
                o.x += (that.center_points[o.zone_id].x - o.x) * k;
            }
        });
    }
};
